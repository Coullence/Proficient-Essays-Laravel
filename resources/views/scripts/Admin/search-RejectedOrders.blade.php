<script>
    $(function() {
        ...
        var searchform = $('#search_RejectedOrders');
        ...
        searchform.submit(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: "{{ route('search_RejectedOrders') }}",
                    ...
                    if (jsonData.length != 0) {
                        $.each(jsonData, function(index, val) {
                            ...
                            let showCellHtml = '<a class="btn btn-sm btn-success btn-block" href="active_orders/' + val.id + '" data-toggle="tooltip" title="{{ trans("usersmanagement.tooltips.show") }}">{!! trans("usersmanagement.buttons.show") !!}</a>';
                             let editCellHtml = '<a class="btn btn-sm btn-info btn-block" href="active_orders/' + val.id + '/edit" data-toggle="tooltip" title="{{ trans("usersmanagement.tooltips.edit") }}">{!! trans("usersmanagement.buttons.edit") !!}</a>'; let deleteCellHtml = '<form method="POST" action="/active_orders/'+ val.id +'" accept-charset="UTF-8" data-toggle="tooltip" title="Delete">' +
                            ...
                            resultsContainer.append('<tr>' +
                                ...
                                '<td>' + val.duration + '</td>' +
                                '<td>' + val.due + '</td>' +
                                '<td> <span class="green">' + val.email + '</span> </td>' +
